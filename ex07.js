...
You've decided that you want to install air pollution sensors around the city to monitor air quality and identify problem areas. We need to write the code for the sensors to trigger a special message when the air is too polluted.
...
It might be useful to start by creating a variable to keep track of how many dirty samples we find as we loop through the array of samples.
We can increment or add to this number using the ++ operator. 
...
const checkAir = function (samples, threshold) {
  // Code here!
  const numberOfDirty = samples.reduce((acc, curr) => {
    if(curr === 'dirty'){
      return acc + 1
    } 
    return acc
