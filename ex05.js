...
If we want to add a parameter to a url, 
there are certain characters that must be encoded in order to make the URL valid. 
There are many characters that must be encoded, including: , !, ", and #. 
For this exercise, we will only be focusing on replacing the space with %20.
...
const urlEncode = function (text) {
  ...
  for(let i = 0; text[i] === ' '; i++){
    editedText = text.slice( i+1 )
  } 
  
  for(let i = editedText.length-1; editedText[i] === ' '; i--){
    editedText = editedText.slice( 0, i )
  }

  const letters = editedText.split('');
